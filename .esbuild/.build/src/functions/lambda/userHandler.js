...
var getAllUsers = async (event, context, callback) => {
  ...
  var result = {};
  result = await documentClient.scan(params).promise().then((data) => {
    return data.Items;
  });
  const response = {
    statusCode: 200,
...
var getAllUsersFunction = middyfy(getAllUsers);
var updateUser = async (event, context, callback) => {
  if (context && event) {
  }
  const body = JSON.parse(event.body);
  var params = {
...
var updateUserFunction = middyfy(updateUser);
var deleteUser = async (event, context, callback) => {
  if (context && event) {
  }
